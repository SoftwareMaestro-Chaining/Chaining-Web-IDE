...
function testsendErrorReport() {
  ...
  assertEquals(
    "/log?script=filename.js&error=message&line=123",
    MockXhrIo.lastUrl
  )
...
function test_nonInternetExplorerSendErrorReport() {
  ...
  stubs.set(goog.global, "setTimeout", function(fcn, time) {
    fcn.call()
  })
...
function testContextProvider() {
  errorReporter = goog.debug.ErrorReporter.install("/errorreporter", function(
    error,
    context
  ) {
