...
function testWindowResize() {
  ...
  assertEquals(
    "No resize expected before the delay is fired",
    0,
    resizeEventCount
  )
  timer.tick(1)
  ...
  assertTrue(
    goog.math.Size.equals(new goog.math.Size(100, 100), bufferedVsm.getSize())
  )
}

function testWindowResize_eventBatching() {
  assertEquals(
    "No resize calls expected before resize events",
    0,
    resizeEventCount
  )
  resize(100, 100)
  ...
  timer.tick(1)
  assertEquals(
    "No resize expected when delay is restarted",
    0,
    resizeEventCount
  )
...
function testWindowResize_noChange() {
  ...
  timer.tick(RESIZE_DELAY)
  assertEquals(
    "No resize expected when size doesn't change",
    1,
    resizeEventCount
  )
  assertTrue(
...
function testWindowResize_previousSize() {
  ...
  assertTrue(
    goog.math.Size.equals(new goog.math.Size(200, 200), bufferedVsm.getSize())
  )
}
