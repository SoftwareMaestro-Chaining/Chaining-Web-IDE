...
function assertUserAgent(expectedAgents, uaString, opt_product, opt_vendor) {
  var mockGlobal = {
    navigator: { userAgent: uaString, product: opt_product, vendor: opt_vendor }
  }
...
function testCompare() {
  assertTrue("exact equality broken", goog.userAgent.compare("1.0", "1.0") == 0)
  assertTrue(
...
function testDocumentModeInStandardsMode() {
  goog.userAgentTestUtil.reinitializeUserAgent()
  var expectedMode = goog.userAgent.IE
    ? parseInt(goog.userAgent.VERSION)
    : undefined
...
function testNoNavigator() {
  ...
  assertEquals(
    "Platform should be the empty string",
    "",
    goog.userAgent.PLATFORM
  )
  assertEquals("Version should be the empty string", "", goog.userAgent.VERSION)
}
