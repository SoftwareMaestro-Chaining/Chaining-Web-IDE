...
goog.debug.DevCss.prototype.getRuleVersionAndCompare_ = function(
  ...
  var comparison = goog.string.compareVersions(
    this.userAgentVersion_,
    ruleVersion
  )
...
goog.debug.DevCss.prototype.addIe6CombinedClassNames_ = function() {
  ...
  for (
    var i = 0, classNameEntry;
    (classNameEntry = this.ie6CombinedMatches_[i]);
    i++
  ) {
    for (var j = 0, el; (el = allEls[j]); j++) {
      ...
      for (
        var k = 0, className;
        (className = classNameEntry.classNames[k]);
        k++
      ) {
    ...
    if (classNameEntry.els.length) {
      for (var j = 0, el; (el = classNameEntry.els[j]); j++) {
        ...
        if (
          !goog.dom.classlist.contains(el, classNameEntry.combinedClassName)
        ) {
