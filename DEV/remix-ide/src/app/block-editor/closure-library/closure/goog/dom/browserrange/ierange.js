 ...
 * @private
 */
goog.dom.browserrange.IeRange.logger_ = goog.log.getLogger(
  "goog.dom.browserrange.IeRange"
)
...
goog.dom.browserrange.IeRange.getBrowserRangeForNodes_ = function(
  ...
  var leftRange = goog.dom.browserrange.IeRange.getBrowserRangeForNode_(
    startNode
  )
  ...
  var rightRange = goog.dom.browserrange.IeRange.getBrowserRangeForNode_(
    endNode
  )
...
goog.dom.browserrange.IeRange.createFromNodeContents = function(node) {
  ...
  if (!goog.dom.browserrange.canContainRangeEndpoint(node)) {
    ...
    range.endNode_ = leaf
    range.endOffset_ =
      leaf.nodeType == goog.dom.NodeType.ELEMENT
...
goog.dom.browserrange.IeRange.prototype.compareNodeEndpoints_ = function(
  ...
  return this.range_.compareEndPoints(
    (thisEndpoint == goog.dom.RangeEndpoint.START ? "Start" : "End") +
      "To" +
      (otherEndpoint == goog.dom.RangeEndpoint.START ? "Start" : "End"),
    goog.dom.browserrange.createRangeFromNodeContents(node).getBrowserRange()
  )
}
...
goog.dom.browserrange.IeRange.prototype.getOffset_ = function(
  ...
  var container =
    opt_container || (isStartEndpoint ? this.getStartNode() : this.getEndNode())
  ...
  if (container.nodeType == goog.dom.NodeType.ELEMENT) {
    ...
    for (var i = edge; i >= 0 && i < len; i += sign) {
      ...
      var endPointCompare = this.compareNodeEndpoints_(
        child,
        endpoint,
        endpoint
      )
...
goog.dom.browserrange.IeRange.prototype.isRangeInDocument = function() {
  ...
  return this.containsRange(
    new goog.dom.browserrange.IeRange(range, this.doc_),
    true
  )
}
