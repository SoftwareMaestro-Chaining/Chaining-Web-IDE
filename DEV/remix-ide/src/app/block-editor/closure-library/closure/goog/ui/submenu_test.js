...
function testLazyInstantiateSubMenu() {
  ...
  assertEquals(
    "Popup menu must be a child of the submenu",
    subMenu,
    lazyMenu.getParent()
  )
  ...
  assertFalse(
    "Popup menu must not have been disposed of",
    lazyMenu.isDisposed()
  )
...
function testReusableMenu() {
  ...
  assertEquals(
    "SubMenuOne must point to the shared menu",
    sharedMenu,
    subMenuOne.getMenu()
  )
  assertEquals(
    "SubMenuOne must be the shared menu's parent",
    subMenuOne,
  ...
  assertEquals(
    "SubMenuTwo must point to the shared menu",
    sharedMenu,
    subMenuTwo.getMenu()
  )
  assertEquals(
    "SubMenuTwo must be the shared menu's parent",
    subMenuTwo,
    sharedMenu.getParent()
  )
  assertEquals(
...
function testSubmenuSelectable() {
  ...
  goog.events.listen(submenu, goog.ui.Component.EventType.ACTION, menuClickedFn)
  ...
  assertEquals(
    "The submenu should not have fired any further events",
    2,
    numClicks
  )
}
...
function assertArrowPosition(subMenu, left) {
  ...
  var expectedLeft = left
    ? 0
    : arrow.offsetParent.offsetWidth - arrow.offsetWidth
