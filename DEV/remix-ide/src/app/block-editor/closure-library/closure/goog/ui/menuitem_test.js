...
function testMenuItem() {
  ...
  assertEquals(
    "Value must default to the caption",
    item.getCaption(),
    item.getValue()
  )
}
...
function testMenuItemConstructor() {
  ...
  assertNotEquals(
    "Value must not equal the caption",
    menuItem.getCaption(),
    menuItem.getValue()
  )
...
function testGetValue() {
  ...
  item.setModel("Foo")
  assertEquals(
    "With a model, value must default to the model",
    item.getModel(),
    item.getValue()
  )
}
...
function testSetValue() {
  ...
  assertEquals(
    "Value and model must be the same",
    item.getValue(),
    item.getModel()
  )
}
...
function testGetSetCaptionAfterCreateDom() {
  ...
  var arrayContent = goog.array.clone(
    goog.dom.safeHtmlToNode(
      goog.html.testing.newSafeHtmlForTest(" <b> \xa0foo</b><i>  bar</i> ")
    ).childNodes
  )
...
function testGetSetContentForItemWithCheckBox() {
  ...
  assertEquals(
    "getContent() must not return the checkbox structure",
    "Item",
    item.getContent()
  )
  ...
  assertEquals(
    "getContent() must return element",
    goog.dom.NodeType.ELEMENT,
    item.getContent().nodeType
  )
  assertTrue(
...
function testGetSetCaptionForItemWithCheckBox() {
  ...
  assertEquals(
    "getCaption() must not return the checkbox structure",
    "Item",
    item.getCaption()
  )
  ...
  item.setContent(goog.dom.createDom(goog.dom.TagName.SPAN, "foo", "Foo"))
  assertEquals(
    "getCaption() must return text content",
    "Foo",
    item.getCaption()
  )
  assertTrue(
...
function testGetSetCaptionForItemWithAccelerators() {
  ...
  assertEquals(
    "getCaption() must not return the accelerator",
    "Hello",
    item.getCaption()
  )
  ...
  assertEquals(
    "getAccelerator() should return the accelerator",
    "Ctrl+1",
    item.getAccelerator()
  )
}
...
function testGetSetCaptionForItemWithMnemonics() {
  ...
  contentArr.push(
    goog.dom.createDom(
      goog.dom.TagName.SPAN,
      goog.getCssName("goog-menuitem-mnemonic-hint"),
      "H"
    )
  )
  ...
  item.setCaption(contentArr)
  assertEquals(
    "getCaption() must not return hint markup",
    "Hello",
    item.getCaption()
  )
  ...
  item.setCaption("")
  assertEquals(
    "getCaption() must return the empty string",
    "",
    item.getCaption()
  )
}
...
function testRenderSelectedItem() {
  ...
  assertTrue(
    ...
    goog.dom.classlist.contains(
      item.getElement(),
      item.getRenderer().getClassForState(goog.ui.Component.State.SELECTED)
    )
  )
}
...
function testRenderCheckedItem() {
  ...
  assertTrue(
    ...
    goog.dom.classlist.contains(
      item.getElement(),
      item.getRenderer().getClassForState(goog.ui.Component.State.CHECKED)
    )
  )
}
...
function testDecorateTemplate() {
  sandbox.innerHTML =
    '<div id="foo" class="goog-menuitem">' +
...
function testSetAriaLabel() {
  ...
  assertEquals(
    "Item element must have expected aria-role",
    "menuitem",
