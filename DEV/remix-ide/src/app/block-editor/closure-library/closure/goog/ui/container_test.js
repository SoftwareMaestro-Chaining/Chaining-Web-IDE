...
function testHighlightOnEnterPreventable() {
  container.decorate(containerElement)
  goog.events.listen(container, goog.ui.Component.EventType.ENTER, function(
    event
  ) {
...
function testGetOwnerControl() {
  ...
  assertNull(
    "Must return null if given null node",
...
function testGetOwnerControlWithNoRenderingInAddChild() {
  ...
  assertEquals(
    "Control was added with addChild(control, false)",
    control,
    ownerControl
  )
}
...
function testHandleKeyEvent_onlyHandlesWhenVisible() {
  ...
  assertEquals(
    "No key events should be handled",
    0,
    keyContainer.keyEventsHandled
  )

  keyContainer.setVisible(true)
  keyContainer.handleKeyEvent(new goog.events.Event())
  assertEquals(
    "One key event should be handled",
    1,
    keyContainer.keyEventsHandled
  )
}
...
function testHandleKeyEvent_alwaysHandlesWithKeyEventTarget() {
  ...
  keyContainer.handleKeyEvent(new goog.events.Event())
  assertEquals(
    "One key events should be handled",
    1,
    keyContainer.keyEventsHandled
  )
}
...
function testHandleKeyEventInternal_onlyHandlesUnmodified() {
  ...
  var event = new goog.events.KeyEvent(goog.events.KeyCodes.ESC, 0, false, null)
...
function testRemoveChild() {
  ...
  assertEquals(
    "Highlighted index must be decreased",
    1,
