...
function testPathologicalInput() {
  ...
  assert(
    goog.string.startsWith(boldTagElArray[0].nextSibling.nodeValue, " Bar...")
  )
}
...
function testMenuFadeDuration() {
  ...
  propertyReplacer.set(
    goog.fx.dom.FadeOutAndHide.prototype,
    "play",
    function() {
      hideAnimCalled = true
    }
...
function testAriaTags() {
  ...
  assertEquals(
    "true",
    goog.a11y.aria.getState(target, goog.a11y.aria.State.EXPANDED)
  )
  assertEquals(
    "true",
    goog.a11y.aria.getState(
...
function testHiliteRowWithDefaultRenderer() {
  ...
  assertTrue(
    goog.dom.classlist.contains(renderer.rowDivs_[2], renderer.activeClassName)
  )
}
...
function assertPreviousNodeText(boldTag, expectedText) {
  ...
  assertEquals(
    "Expected text before the token does not match",
    expectedText,
    prevNode.nodeValue
  )
}
...
function assertLastNodeText(node, expectedText) {
  ...
  assertEquals(
    "Incorrect text in the last node",
    expectedText,
    lastNode.nodeValue
  )
}
