...
goog.date.relative.format = function(dateMs) {
  ...
  if (delta < 60) {
    ...
    return goog.date.relative.formatTimeDelta_(
      delta,
      future,
      goog.date.relative.Unit.MINUTES
    )
  } else {
...
goog.date.relative.formatDay = function(dateMs, opt_formatter) {
  ...
  var dayAfterTomorrow = new Date(
    today.getTime() + 2 * goog.date.relative.DAY_MS_
  )
...
goog.date.relative.getDateString_ = function(
  ...
  if (delta < 60 * 24) {
    ...
    return (
      (opt_shortTimeMsg || goog.date.relative.formatShortTime_(date)) +
      relativeDate
    )
  } else {
    return (
      (opt_fullDateMsg || goog.date.relative.formatFullDate_(date)) +
      relativeDate
    )
  }
}
...
goog.date.relative.getMessage_ = function(delta, future, unit) {
  ...
  if (!future && unit == goog.date.relative.Unit.MINUTES) {
     ...
     * (singular).
     */
    var MSG_MINUTES_AGO_SINGULAR = goog.getMsg("{$num} minute ago", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_MINUTES_AGO_PLURAL = goog.getMsg("{$num} minutes ago", {
      num: deltaFormatted
    })
     ...
     * (singular).
     */
    var MSG_IN_MINUTES_SINGULAR = goog.getMsg("in {$num} minute", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_IN_MINUTES_PLURAL = goog.getMsg("in {$num} minutes", {
      num: deltaFormatted
    })
     ...
     * (singular).
     */
    var MSG_HOURS_AGO_SINGULAR = goog.getMsg("{$num} hour ago", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_HOURS_AGO_PLURAL = goog.getMsg("{$num} hours ago", {
      num: deltaFormatted
    })
     ...
     * (singular).
     */
    var MSG_IN_HOURS_SINGULAR = goog.getMsg("in {$num} hour", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_IN_HOURS_PLURAL = goog.getMsg("in {$num} hours", {
      num: deltaFormatted
    })
     ...
     * (singular).
     */
    var MSG_DAYS_AGO_SINGULAR = goog.getMsg("{$num} day ago", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_DAYS_AGO_PLURAL = goog.getMsg("{$num} days ago", {
      num: deltaFormatted
    })
     ...
     * (singular).
     */
    var MSG_IN_DAYS_SINGULAR = goog.getMsg("in {$num} day", {
      num: deltaFormatted
    })
     ...
     * (plural).
     */
    var MSG_IN_DAYS_PLURAL = goog.getMsg("in {$num} days", {
      num: deltaFormatted
    })
