...
function testIsValidRgbColor() {
  ...
  for (var i = 0; i < goodRgbColors.length; i++) {
    assertEquals(
      goodRgbColors[i],
      goog.color.isValidRgbColor_(goodRgbColors[i]).length,
      3
    )
  }
  for (var i = 0; i < badRgbColors.length; i++) {
    assertEquals(
      badRgbColors[i],
      goog.color.isValidRgbColor_(badRgbColors[i]).length,
      0
    )
  }
}
...
function testHSLBidiToRGB() {
  ...
  for (var i = 0; i < color.length; i++) {
    ...
    colorConversionTestHelper(
      function(color) {
        return goog.color.rgbArrayToHsl(color)
      },
      function(color) {
...
function testHSVToRGB() {
  ...
  for (var i = 0; i < color.length; i++) {
    ...
    colorConversionTestHelper(
      function(color) {
        return goog.color.rgbArrayToHsv(color)
      },
      function(color) {
        return goog.color.hsvArrayToRgb(color)
      },
      color[i],
      DELTA
    )
...
function testHslToHex() {
  ...
  var color = [[0, 0, 0], [20, 0.5, 0.5], [0, 0, 1], [255, 0.45, 0.76]]
  ...
  for (var i = 0; i < color.length; i++) {
    colorConversionTestHelper(
      ...
      function(hex) {
        return goog.color.hexToHsl(hex)
      },
      color[i],
      DELTA
    )
    ...
    colorConversionTestHelper(
      function(hsl) {
        return goog.color.hslArrayToHex(hsl)
      },
      function(hex) {
        return goog.color.hexToHsl(hex)
      },
...
function testHsvToHex() {
  ...
  var color = [[0, 0, 0], [0.5, 0.5, 155], [0, 0, 255], [0.7, 0.45, 21]]

  for (var i = 0; i < color.length; i++) {
    colorConversionTestHelper(
      function(hsl) {
        return goog.color.hsvToHex(hsl[0], hsl[1], hsl[2])
      },
      function(hex) {
        return goog.color.hexToHsv(hex)
      },
      color[i],
      DELTA
    )
    ...
    colorConversionTestHelper(
      function(hsl) {
        return goog.color.hsvArrayToHex(hsl)
      },
      function(hex) {
        return goog.color.hexToHsv(hex)
      },
...
function testColorBlend() {
  ...
  // same colors.
  var greenWithGreen = goog.color.blend(green, green, 0.25)
  assertTrue(
  ...
  var blackWithWhite = goog.color.blend(black, white, 0.5)
  assertTrue(
    "black + .5 * white = gray",
    rgbColorsAreEqual(gray, blackWithWhite)
  )
  var redAndBlue = goog.color.blend(red, blue, 0.5)
  assertTrue("red + .5 * blue = purple", rgbColorsAreEqual(purple, redAndBlue))
  var lightGreen = goog.color.blend(green, white, 0.75)
  assertTrue(
...
function testColorDarken() {
  ...
  var darkBlack = goog.color.darken(black, 0.63)
  assertTrue(
  ...
  var whiteDarkened = goog.color.darken(white, 0.43)
  var whiteDarkenedHsl = goog.color.rgbToHsl(
    whiteDarkened[0],
    whiteDarkened[1],
    whiteDarkened[2]
  )
  ...
  var purpleDarkened = goog.color.darken(purple, 0.1)
...
function testColorLighten() {
  ...
  var lightWhite = goog.color.lighten(white, 0.41)
  assertTrue(
  ...
  var blackLightened = goog.color.lighten(black, 0.33)
  ...
  var orangeLightened = goog.color.lighten(orange, 0.91)
...
function testYiqBrightness() {
  ...
  assertTrue(
    "White brightness is a number.",
    typeof whiteBrightness == "number"
  )
  assertTrue(
    "Coral brightness is a number.",
...
function testYiqBrightnessDiff() {
  ...
  for (var name1 in colors) {
    for (var name2 in colors) {
      diffs[name1 + "-" + name2] = goog.color.yiqBrightnessDiff_(
        colors[name1],
        colors[name2]
      )
    }
  ...
  assertEquals(
    "saddlebrown-indigo is also 47.",
    diffs["saddlebrown-indigo"],
    47
  )
}
...
function testColorDiff() {
  ...
  for (var name1 in colors) {
    for (var name2 in colors) {
      diffs[name1 + "-" + name2] = goog.color.colorDiff_(
        colors[name1],
        colors[name2]
      )
    }
  ...
  assertEquals(
    "mediumblue-orchid is also 339.",
    diffs["mediumblue-orchid"],
    339
  )
}
...
function testHighContrast() {
  ...
  var suggestion = goog.color.highContrast(black, [
    white,
    black,
    sienna,
    lemonchiffron
