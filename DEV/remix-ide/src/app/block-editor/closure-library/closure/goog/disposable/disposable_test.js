...
function testOnDisposeCallback() {
  ...
  d1.dispose()
  assertEquals(
    "callback should be called once on dispose",
    1,
...
function testOnDisposeCallbackOrder() {
  ...
  goog.dispose(d1)
  assertArrayEquals(
    "callbacks should be called in chronological order",
    ["a", "b"],
    invocations
  )
}
...
function testAddOnDisposeCallbackAfterDispose() {
  ...
  assertEquals(
    "Callback scope should be respected",
    scope,
...
function testInteractiveMonitoring() {
  ...
  d2.dispose()
  assertSameElements("all disposed", [], goog.Disposable.getUndisposedObjects())
}
