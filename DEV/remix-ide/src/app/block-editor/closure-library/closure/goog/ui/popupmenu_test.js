...
function testIsAttachTarget() {
  ...
  popup.attach(anchor)
  assertTrue(
    "Menu should be attached to the anchor",
    popup.isAttachTarget(anchor)
  )
...
function testGetAttachTarget() {
  ...
  assertTrue(
    "Failed to get target after attaching element to menu",
    target != null
  )
...
function testSmallViewportSliding() {
  ...
  var rightwardPos = Math.floor(
    ((viewportRect.right - viewportRect.left) / 3) * 2
  )
  ...
  assertObjectEquals(
    ...
    new goog.math.Coordinate(
      viewportRect.right - viewportRect.left - largeWidth,
      0
    ),
    goog.style.getPosition(popup.getElement())
  )
