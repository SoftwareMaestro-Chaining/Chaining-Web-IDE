...
function testGetElementsByTagNameAndClass() {
  assertEquals(
    "Should get 6 spans",
    goog.dom.getElementsByTagNameAndClass(goog.dom.TagName.SPAN).length,
    6
  )
  assertEquals(
    "Should get 6 spans",
  ...
  assertEquals(
    1,
    goog.dom.getElementsByTagNameAndClass(undefined, "mixedCaseClass").length
  )
...
function testCreateDom() {
  var el = goog.dom.createDom(
    ...
    goog.dom.createDom(
      goog.dom.TagName.P,
      { style: "font: normal 12px arial; color: red; " },
      "Para 1"
    ),
    goog.dom.createDom(
      goog.dom.TagName.P,
      { style: "font: bold 18px garamond; color: blue; " },
      "Para 2"
    ),
    goog.dom.createDom(
      goog.dom.TagName.P,
...
function testCreateDomAcceptsArray() {
  ...
  assertEquals(
    "First child should be an LI tag",
    String(goog.dom.TagName.LI),
...
function testCreateDomStringArg() {
  ...
  assertEquals(
    "firstChild should be a text node",
...
function testCreateDomNodeListArg() {
  ...
  var simpleElem = goog.dom.createDom(
    goog.dom.TagName.DIV,
    null,
    "Hello, world!"
  )
...
function testFlattenElement() {
  ...
  assertEquals(
    "Previous sibling of br should be text",
    text,
    br.previousSibling
  )
  ...
  assertEquals(
    "outOfDoc should still have 1 child",
    1,
...
function testIsNodeLike() {
  ...
  assertTrue(
    "document.body should be node like",
  ...
  assertTrue(
    "custom object should be node like",
...
function testIsWindow() {
  ...
  try {
    ...
    if (otherWindow) {
      assertTrue(
        "other window should be a window",
    ...
    assertFalse(
      "undefined should not be a window",
...
function testGetChildren() {
  ...
  assertEquals(
    "List of element children should be length two.",
    2,
    children.length
  )
  ...
  assertEquals(
    "List of element children should be length zero.",
    0,
    noChildren.length
  )
...
function testGetNextNode() {
  var tree = goog.dom.safeHtmlToNode(
...
function testSetTextContent() {
  ...
  assertEquals(
    "We should have one childNode after setTextContent",
    1,
...
function testFindNode() {
  ...
  var result = goog.dom.findNode(document, function(n) {
    return (
      n.nodeType == goog.dom.NodeType.ELEMENT &&
  ...
  result = goog.dom.findNode(document, function(n) {
    return (
      n.nodeType == goog.dom.NodeType.ELEMENT && n.tagName == goog.dom.TagName.P
    )
  })
  assertEquals(expected, result)
...
function testIsFocusable() {
  ...
  assertTrue(
    "isFocusable() must be true for anchor elements with " + "no tab index",
    goog.dom.isFocusable(goog.dom.getElement("noTabIndexAnchor"))
  )
  assertTrue(
    "isFocusable() must be true for input elements with " + "no tab index",
    goog.dom.isFocusable(goog.dom.getElement("noTabIndexInput"))
  )
  assertTrue(
    "isFocusable() must be true for textarea elements with " + "no tab index",
    goog.dom.isFocusable(goog.dom.getElement("noTabIndexTextArea"))
  )
  assertTrue(
    "isFocusable() must be true for select elements with " + "no tab index",
    goog.dom.isFocusable(goog.dom.getElement("noTabIndexSelect"))
  )
  assertTrue(
    "isFocusable() must be true for button elements with " + "no tab index",
  ...
  assertTrue(
    "isFocusable() must be true for form elements with " + "zero tab index",
    goog.dom.isFocusable(goog.dom.getElement("zeroTabIndexButton"))
  )

  // Test form element with positive tab index is focusable.
  assertTrue(
    "isFocusable() must be true for form elements with " + "positive tab index",
...
function testGetNodeTextLength() {
  ...
  assertEquals(
    10,
    goog.dom.getNodeTextLength(createTestDom("a<b>bcd</b><br />efghi"))
  )
}
...
function testGetNodeTextOffset() {
  assertEquals(
    4,
    goog.dom.getNodeTextOffset($("offsetTest1"), $("offsetParent1"))
  )
...
function testCanHaveChildren() {
  ...
  var IE_ILLEGAL_ELEMENTS = goog.object.createSet(
    goog.dom.TagName.APPLET,
    goog.dom.TagName.EMBED
  )
...
function testGetAncestorNoElement() {
  assertNull(
    goog.dom.getAncestor(null /* element */, goog.functions.TRUE /* matcher */)
  )
  assertNull(
    goog.dom.getAncestor(
      null /* element */,
      goog.functions.TRUE /* matcher */,
...
function testGetAncestorNoMatch() {
  ...
  assertNull(
    goog.dom.getAncestor(elem, function() {
      return false
    })
  )
}
...
function testCreateTable() {
  ...
  assertEquals(
    2,
    goog.dom.getElementsByTagName(goog.dom.TagName.TR, table).length
  )
  assertEquals(
    3,
    goog.dom.getElementsByTagName(goog.dom.TagName.TR, table)[0].childNodes
      .length
  )
  assertEquals(
    6,
    goog.dom.getElementsByTagName(goog.dom.TagName.TD, table).length
  )
  assertEquals(
    goog.string.Unicode.NBSP,
    goog.dom.getElementsByTagName(goog.dom.TagName.TD, table)[0].firstChild
      .nodeValue
  )
  ...
  assertEquals(
    0,
    goog.dom.getElementsByTagName(goog.dom.TagName.TD, table)[0].childNodes
      .length
  )
}
...
function assertConstHtmlToNodeStringifiesToOneOf(
  potentialStringifications,
  var_args
) {
  var node = goog.dom.constHtmlToNode.apply(
    undefined,
    goog.array.slice(arguments, 1)
  )
  var stringified = goog.dom.getOuterHtml(node)
  if (
    goog.array.find(potentialStringifications, function(element) {
    ...
    fail(
      'Unexpected stringification for a node built from "' +
        goog.array
          .map(goog.array.slice(arguments, 1), goog.string.Const.unwrap)
          .join("") +
        '": "' +
        stringified +
        '"'
...
function testRegularConstHtmlToNodeStringifications() {
  assertConstHtmlToNodeStringifiesToOneOf(
    ["<b>foo</b>", "<B>foo</B>"],
    goog.string.Const.from("<b>foo</b>")
  )

  assertConstHtmlToNodeStringifiesToOneOf(
    ["<br>", "<BR>"],
    goog.string.Const.from("<br>")
  )
