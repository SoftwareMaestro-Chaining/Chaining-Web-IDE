...
function testNormalize() {
  ...
  goog.vec.vec3d.scale(v0, 1 / goog.vec.vec3d.magnitude(v0), v2)
...
function testLerp() {
  ...
  goog.vec.vec3d.lerp(v0, v1, 0.5, v2)
...
function testSlerp() {
  ...
  goog.vec.vec3d.slerp(v0, v1, 0.5, v5)
  assertElementsRoughlyEqual(
  ...
  v5 = goog.vec.vec3d.slerp(v0, v4, 0.75, v5)
  assertRoughlyEquals(
    (Math.PI * 3) / 4,
    Math.acos(goog.vec.vec3d.dot(v5, v0)),
    goog.vec.EPSILON
  ...
  v5 = goog.vec.vec3d.slerp(v1, v2, 0.5, v5)
  // Axis of rotation is arbitrary, but result should be 90 degrees from both
  // v1 and v2 when f = 0.5.
  assertRoughlyEquals(
    Math.PI / 2,
    Math.acos(goog.vec.vec3d.dot(v5, v1)),
    goog.vec.EPSILON
  )
  assertRoughlyEquals(
    Math.PI / 2,
    Math.acos(goog.vec.vec3d.dot(v5, v2)),
    goog.vec.EPSILON
  )

  // f == 0.25, result should be 45-degrees to v1, and 135 to v2.
  v5 = goog.vec.vec3d.slerp(v1, v2, 0.25, v5)
  assertRoughlyEquals(
    Math.PI / 4,
    Math.acos(goog.vec.vec3d.dot(v5, v1)),
    goog.vec.EPSILON
  ...
  v5 = goog.vec.vec3d.slerp(v1, v2, 0.75, v5)
  assertRoughlyEquals(
    (Math.PI * 3) / 4,
    Math.acos(goog.vec.vec3d.dot(v5, v1)),
    goog.vec.EPSILON
  ...
  goog.vec.vec3d.slerp(v1, v3, 0.5, v5)
  // Axis of rotation is arbitrary, but result should be 90 degrees from both
  // v1 and v3.
  assertRoughlyEquals(
    Math.PI / 2,
    Math.acos(
      goog.vec.vec3d.dot(v5, v1) /
        (goog.vec.vec3d.magnitude(v5) * goog.vec.vec3d.magnitude(v1))
    ),
    goog.vec.EPSILON
  )
  assertRoughlyEquals(
    Math.PI / 2,
    Math.acos(
      goog.vec.vec3d.dot(v5, v3) /
        (goog.vec.vec3d.magnitude(v3) * goog.vec.vec3d.magnitude(v5))
    ),
    goog.vec.EPSILON
  )
  ...
  var v7 = goog.vec.vec3d.setFromValues(
    goog.vec.vec3d.create(),
    0.009,
    0.147,
    0.989
  )
  goog.vec.vec3d.slerp(v7, v7, 0.25, v5)
