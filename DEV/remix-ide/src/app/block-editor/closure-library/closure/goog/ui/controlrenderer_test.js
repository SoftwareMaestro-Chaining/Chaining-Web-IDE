...
function testConstructor() {
  assertNotNull(
    "ControlRenderer singleton instance must not be null",
    controlRenderer
  )
  assertNotNull(
    "TestRenderer singleton instance must not be null",
    testRenderer
  )
}
...
function testDecorateOptimization() {
  ...
  // called at all.
  sandbox.innerHTML =
    '<div id="bar" class="goog-base goog-button">Bar' + "</div>"
  ...
  // called at all.
  sandbox.innerHTML =
    '<div id="baz" class="goog-base goog-button ' +
...
function testSetAriaRole() {
  ...
  assertEquals(
    "Element must have expected ARIA role",
    goog.a11y.aria.Role.BUTTON,
...
function testSetAllowTextSelection() {
  ...
  assertFalse(
    "Parent element must be selectable",
    goog.style.isUnselectable(foo)
  )
  assertFalse(
...
function testUpdateAriaStateOpened() {
  ...
  var element = control.getElement()
  assertNotNull(element)
  controlRenderer.updateAriaState(element, goog.ui.Component.State.OPENED, true)
  assertEquals(
...
function testSetAriaStateRoleNotInMap() {
  ...
  control.decorate(element)
  assertEquals(
    "Element should have ARIA role option.",
    goog.a11y.aria.Role.OPTION,
...
function testSetAriaStateRoleInMapMatches() {
  ...
  control.decorate(element)
  assertEquals(
    "Element should have ARIA role checkbox.",
...
function testSetAriaStateRoleInMapNotMatches() {
  ...
  control.decorate(element)
  assertEquals(
    "Element should have ARIA role button.",
    goog.a11y.aria.Role.BUTTON,
    goog.a11y.aria.getRole(element)
  )
  control.setStateInternal(goog.ui.Component.State.DISABLED, true)
...
function testSetContentArray() {
  ...
  assertEquals(
    "1st child must be a text node",
    goog.dom.NodeType.TEXT,
    sandbox.childNodes[0].nodeType
  )
  assertEquals(
    "2nd child must be an element",
    goog.dom.NodeType.ELEMENT,
    sandbox.childNodes[1].nodeType
  )
  assertEquals(
    "3rd child must be a text node",
    goog.dom.NodeType.TEXT,
