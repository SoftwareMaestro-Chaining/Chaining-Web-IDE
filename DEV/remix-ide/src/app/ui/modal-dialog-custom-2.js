...
module.exports = {
  ...
  alert: function(text) {
    modal("", yo`<p>${text}</p>`, null, { label: null })
  },
  ...
  promptPassphraseCreation: function(ok, cancel) {
    ...
    modal(
      null,
      yo`<p>${text}<p>${input}</p></p>`,
      {
        fn: () => {
          if (typeof ok === "function") {
            if (
              input.querySelector("#prompt1").value ===
              input.querySelector("#prompt2").value
            ) {
  ...
  promptMulti: function({ title, text, inputValue }, ok, cancel) {
    ...
    var input = yo`<textarea id="prompt_text" class=${
      css.prompt_text
    } rows="4" cols="50"></textarea>`
    modal(
      title,
      yo`<div>${text}<div>${input}</div></div>`,
      {
        fn: () => {
          if (typeof ok === "function")
            ok(document.getElementById("prompt_text").value)
        }
      },
      {
        fn: () => {
...
function prompt(title, text, hidden, inputValue, ok, cancel, focus) {
  ...
  var input = yo`<input type=${type} name='prompt_text' id='prompt_text' class="${
    css["prompt_text"]
  }" value='${inputValue}' >`
  modal(
    title,
