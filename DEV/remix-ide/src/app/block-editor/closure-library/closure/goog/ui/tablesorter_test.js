...
function setUp() {
  ...
  notSortableHeader = goog.dom.getElementsByTagName(
    goog.dom.TagName.TH,
    table
  )[2]
...
function testBackwardAlpha() {
  ...
  assertTrue(
    goog.dom.classlist.contains(alphaHeader, "goog-tablesorter-sorted-reverse")
  )
  assertEquals(0, tableSorter.getSortColumn())
...
function testBackwardNumeric() {
  ...
  assertTrue(
    goog.dom.classlist.contains(numberHeader, "goog-tablesorter-sorted-reverse")
  )
  assertEquals(1, tableSorter.getSortColumn())
  assertTrue(tableSorter.isSortReversed())
}
