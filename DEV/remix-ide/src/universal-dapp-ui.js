...
UniversalDAppUI.prototype.renderInstanceFromABI = function(
  ...
  var instance = yo`<div class="instance ${css.instance} ${
    css.hidesub
  }" id="instance${address}"></div>`
  ...
  <div class="${
    css.titleText
  }"> ${contractName} at ${shortAddress} (${context}) </div>
  ${copyToClipboard(() => address)}
  </div>`

  if (self.udapp.removable_instances) {
    var close = yo`<div class="${css.udappClose}" onclick=${remove}><i class="${
      css.closeIcon
  ...
  $.each(contractABI, (i, funABI) => {
    ...
    methods +=
      "contractInstance.methods." + funABI.name + "(args[" + index + "])\n"
