...
function testNoDisabledButtonFocus() {
  ...
  var buttonEl = dialog
    .getButtonSet()
    .getButton(goog.ui.Dialog.DefaultButtonKeys.OK)
...
function testEscapeKeyDoesNothingOnSpecialFormElements() {
  dialog.setSafeHtmlContent(
    goog.html.SafeHtml.create("select", {}, [
...
function testKeydownClosesWithoutButtonSet() {
  ...
  dialog.setSafeHtmlContent(
    goog.html.SafeHtml.create("button", { id: "button", name: "ok" }, "OK")
  )
  var wasCalled = false
  function called() {
    wasCalled = true
...
function testSetButtonEnabled() {
  ...
  assertFalse(buttonSet.getButton(goog.ui.Dialog.DefaultButtonKeys.NO).disabled)
...
function testIframeMask() {
  var prevNumFrames = goog.dom.getElementsByTagNameAndClass(
  ...
  var curNumFrames = goog.dom.getElementsByTagNameAndClass(
    goog.dom.TagName.IFRAME
  ).length
...
function testNonModalDialog() {
  ...
  assertEquals(
    "Iframe mask created for modal dialog",
    prevNumFrames,
    curNumFrames
  )
}
...
function testDraggableStyle() {
  assertTrue(
    ...
    goog.dom.classlist.contains(dialog.titleEl_, "modal-dialog-title-draggable")
  )
...
function testDraggingLifecycle() {
  ...
  dialog.setVisible(true)
  assertNotNull(
    "dragger is created when the dialog is rendered",
    dialog.dragger_
  )
...
function testHtmlContent() {
  dialog.setSafeHtmlContent(
    goog.html.testing.newSafeHtmlForTest('<span class="theSpan">Hello</span>')
  )
  var spanEl = goog.dom.getElementByClass("theSpan", dialog.getContentElement())
