...
goog.crypt.hashTester.runBasicTests = function(hash) {
  ...
  hash.update([97, 158])
  assertArrayEquals(
    "The reset did not produce the initial state",
    golden1,
    hash.digest()
  )
  ...
  hash.update([97, 66])
  assertArrayEquals(
    "String and array inputs should give the same result",
    golden2,
    hash.digest()
  )
  ...
  hash.update([158])
  assertArrayEquals(
    "Partial updates resulted in a different hash",
    golden1,
    hash.digest()
  )
  ...
  assertArrayEquals(
    "Updating with an explicit buffer length did not work",
    golden1,
    hash.digest()
  )
}
...
goog.crypt.hashTester.runPerfTests = function(hashFactory, hashName) {
  ...
  function runPerfTest(byteLength, updateCount) {
    var label =
      hashName + ": " + updateCount + " update(s) of " + byteLength + " bytes"
