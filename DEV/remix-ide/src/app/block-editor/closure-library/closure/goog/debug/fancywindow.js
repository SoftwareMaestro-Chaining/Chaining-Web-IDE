...
goog.debug.FancyWindow.prototype.writeInitialDocument = function() {
  ...
  ;(goog.userAgent.IE ? doc.body : this.win).onresize = goog.bind(
    this.resizeStuff_,
    this
  )
  ...
  this.dh_.getElement("closebutton").onclick = goog.bind(
    this.closeOptions_,
    this
  )
...
goog.debug.FancyWindow.prototype.closeOptions_ = function() {
  ...
  for (var i = 0; i < loggers.length; i++) {
    var logger = loggers[i]
    var sel = /** @type {!HTMLSelectElement} */ (dh.getElement(
      "sel" + logger.getName()
    ))
...
goog.debug.FancyWindow.prototype.resizeStuff_ = function() {
  ...
  logel.style.height =
    dh.getDocument().body.offsetHeight -
    headel.offsetHeight -
    (goog.userAgent.IE ? 4 : 0) +
    "px"
}
...
goog.debug.FancyWindow.prototype.getHtml_ = function() {
  ...
  var body = SafeHtml.create(
    ...
    SafeHtml.concat(
      SafeHtml.create("div", {
        id: "log",
        style: goog.string.Const.from("overflow:auto")
      }),
      SafeHtml.create(
        "div",
        { id: "head" },
...
goog.debug.FancyWindow.getStoredKeys_ = function() {
  ...
  for (var i = 0, len = window.localStorage.length; i < len; i++) {
    ...
    if (
      key != null &&
      goog.string.startsWith(key, goog.debug.FancyWindow.LOCAL_STORE_PREFIX)
    ) {
      storedKeys[key] = true
    }
  }
