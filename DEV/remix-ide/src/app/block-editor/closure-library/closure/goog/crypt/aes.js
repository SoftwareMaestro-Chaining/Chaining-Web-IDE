...
goog.crypt.Aes.prototype.copyInput_ = function(input) {
  ...
  goog.asserts.assert(
    input.length == this.BLOCK_SIZE,
    "Expecting input of block size."
  )
...
goog.crypt.Aes.prototype.shiftRows_ = function() {
  ...
  for (var r = 1; r < 4; r++) {
    for (var c = 0; c < 4; c++) {
      this.state_[r][c] = this.temp_[r][
        (c + r) % goog.crypt.Aes.BLOCK_SIZE_IN_WORDS_
      ]
    }
  }
}
...
goog.crypt.Aes.prototype.mixColumns_ = function() {
  ...
  for (var c = 0; c < 4; c++) {
    ...
    s[0][c] =
      goog.crypt.Aes.MULT_2_[t[0]] ^ goog.crypt.Aes.MULT_3_[t[1]] ^ t[2] ^ t[3]
    s[1][c] =
      t[0] ^ goog.crypt.Aes.MULT_2_[t[1]] ^ goog.crypt.Aes.MULT_3_[t[2]] ^ t[3]
    s[2][c] =
      t[0] ^ t[1] ^ goog.crypt.Aes.MULT_2_[t[2]] ^ goog.crypt.Aes.MULT_3_[t[3]]
    s[3][c] =
      goog.crypt.Aes.MULT_3_[t[0]] ^ t[1] ^ t[2] ^ goog.crypt.Aes.MULT_2_[t[3]]
  }
}
...
goog.crypt.Aes.prototype.keyExpansion_ = function() {
  ...
  for (
    ...
    if (rowNum % this.keyLengthInWords_ == 0) {
      ...
      this.keyLengthInWords_ > 6 &&
      rowNum % this.keyLengthInWords_ == 4
    ) {
      this.subWord_(temp)
    }
