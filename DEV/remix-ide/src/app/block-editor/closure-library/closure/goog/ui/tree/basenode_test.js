...
function testExpandEvents() {
  ...
  var callCount = 0
  n.addEventListener(goog.ui.tree.BaseNode.EventType.BEFORE_EXPAND, function(
    e
  ) {
    assertEquals(expanded, n.getExpanded())
    callCount++
...
function testExpandEvents2() {
  ...
  assertEquals(expanded, n.getExpanded())
  var callCount = 0
  n.addEventListener(goog.ui.tree.BaseNode.EventType.BEFORE_COLLAPSE, function(
    e
  ) {
    assertEquals(expanded, n.getExpanded())
    callCount++
...
function testRemoveChild() {
  ...
  assertFalse(
    "nodeA should be removed from tree of node1",
    node1.contains(nodeA)
  )
  assertTrue(
...
function testGetPreviousShownNode() {
  ...
  node1.expand()
  assertEquals(
    "prev node for node3 when node1 is expanded",
    node2,
    node3.getPreviousShownNode()
  )
  assertEquals(
    "prev node for node2 when node1 is expanded",
    node1,
    node2.getPreviousShownNode()
  )
  assertEquals(
    "prev node for node1 when root is shown",
    tree,
...
function testInvisibleNodesInUnrenderedTree() {
  ...
  var textContent = tree.getElement().textContent || tree.getElement().innerText
  assertContains("Node should be rendered.", "tree", textContent)
  assertContains("Node should be rendered.", "a", textContent)
  assertNotContains(
    "Unexpanded node child should not be rendered.",
    "b",
    textContent
  )
