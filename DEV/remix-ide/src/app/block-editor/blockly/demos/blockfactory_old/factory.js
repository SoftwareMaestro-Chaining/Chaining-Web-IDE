...
function formatJson_(blockType, rootBlock) {
  ...
  switch (rootBlock.getFieldValue("CONNECTIONS")) {
    case "LEFT":
      JS.output = JSON.parse(getOptTypesFrom(rootBlock, "OUTPUTTYPE") || "null")
      break
    case "BOTH":
  ...
  for (var i = 0, input; (input = block.inputList[i]); i++) {
    for (var j = 0, field; (field = input.fieldRow[j]); j++) {
      ...
      if (field instanceof Blockly.FieldVariable) {
        ...
        code.push(
          makeVar("date", name) + " = block.getFieldValue('" + name + "');"
        )
...
function init() {
  ...
  if ("BlocklyStorage" in window && window.location.hash.length > 1) {
    BlocklyStorage.retrieveXml(window.location.hash.substring(1), mainWorkspace)
  } else {
    var xml =
  ...
  document.getElementById("languageTA").addEventListener("keyup", updatePreview)
