...
function setUp() {
  zippy = new goog.ui.Zippy(
    goog.dom.getElement("t1"),
  ...
  contentlessZippy = new goog.ui.Zippy(
    fakeControlEl.cloneNode(true),
    undefined,
    true
  )
  headerlessZippy = new goog.ui.Zippy(null, fakeContentEl.cloneNode(true), true)
...
function testExpandCollapse_lazyZippy() {
  ...
  assertEquals("expanded must be false", false, lazyZippy.isExpanded())
  assertEquals(
    "contentEl should not be visible",
    "none",
    lazyZippyContentEl.style.display
  )
...
function testCssClassesAndAria() {
  ...
  assertTrue(
    ...
    goog.dom.classlist.contains(zippy.getContentElement(), "goog-zippy-content")
  )
  assertEquals(
    "header aria role is TAB",
    "tab",
...
function testActionEvent() {
  ...
  var handleEvent = function(e) {
    if (e.type == goog.ui.Zippy.Events.TOGGLE) {
      ...
      actionEventCount++
      assertNotNull(
        "action event must have triggering event",
        e.triggeringEvent
      )
      assertTrue(
...
function testBasicZippyBehavior() {
  ...
  goog.testing.events.fireClickSequence(zippy.elHeader_)
  assertEquals(
    "Zippy  must have dispatched TOGGLE on click",
    1,
    dispatchedActionCount
  )
}
