...
WorkspaceFactoryController.prototype.addCategory = function() {
  ...
  // Get name from user.
  var name = this.promptForNewCategoryName(
...
WorkspaceFactoryController.prototype.printConfig = function() {
  ...
  // Print XML.
  window.console.log(
    Blockly.Xml.domToPrettyText(this.generator.generateToolboxXml())
  )
}
...
WorkspaceFactoryController.prototype.changeCategoryName = function() {
  ...
  window.foo = selected
  var newName = this.promptForNewCategoryName(
    "What do you want to change this" + " category's name to?",
    selected.name
  )
  if (!newName) {
    // If cancelled.
 ...
 * a valid CSS string.
 */
WorkspaceFactoryController.prototype.changeSelectedCategoryColor = function(
  color
) {
...
WorkspaceFactoryController.prototype.clearAll = function() {
  var msg =
    "Are you sure you want to clear all of your work in Workspace" + " Factory?"
  BlocklyDevTools.Analytics.onWarning(msg)
  if (!confirm(msg)) {
    return
  }
...
WorkspaceFactoryController.prototype.clearAndLoadXml_ = function(xml) {
  ...
  this.view.markShadowBlocks(
    this.model.getShadowBlocksInWorkspace(this.toolboxWorkspace.getAllBlocks())
  )
...
WorkspaceFactoryController.prototype.generateNewOptions = function() {
  ...
  this.reinjectPreview(
    Blockly.Options.parseToolboxTree(this.generator.generateToolboxXml())
  )
}
...
WorkspaceFactoryController.prototype.readOptions_ = function() {
  ...
  if (readonly) {
    ...
    if (document.getElementById("option_infiniteBlocks_checkbox").checked) {
      ...
      var maxBlocksValue = document.getElementById("option_maxBlocks_number")
        .value
  ...
  optionsObj["sounds"] = document.getElementById(
    "option_sounds_checkbox"
  ).checked
  ...
  if (document.getElementById("option_grid_checkbox").checked) {
    ...
    var spacingValue = document.getElementById("gridOption_spacing_number")
      .value
    grid["spacing"] =
  ...
  if (document.getElementById("option_zoom_checkbox").checked) {
    ...
    var startScaleValue = document.getElementById(
      "zoomOption_startScale_number"
    ).value
    zoom["startScale"] =
      typeof startScaleValue == "string"
        ? parseFloat(startScaleValue)
        : startScaleValue
    var maxScaleValue = document.getElementById("zoomOption_maxScale_number")
      .value
    zoom["maxScale"] =
      typeof maxScaleValue == "string"
        ? parseFloat(maxScaleValue)
        : maxScaleValue
    var minScaleValue = document.getElementById("zoomOption_minScale_number")
      .value
    zoom["minScale"] =
      typeof minScaleValue == "string"
        ? parseFloat(minScaleValue)
        : minScaleValue
    var scaleSpeedValue = document.getElementById(
      "zoomOption_scaleSpeed_number"
    ).value
    zoom["scaleSpeed"] =
      typeof scaleSpeedValue == "string"
        ? parseFloat(scaleSpeedValue)
        : scaleSpeedValue
...
WorkspaceFactoryController.prototype.importBlocks = function(file, format) {
  ...
  reader.onload = function() {
    try {
      ...
      var blockTypes = FactoryUtils.defineAndGetBlockTypes(
        reader.result,
        format
      )
  ...
  for (var i = 0, block; (block = blocks[i]); i++) {
    if (!this.isDefinedBlock(block)) {
      block.setWarningText(
        block.type +
          " is not defined (it is not a standard " +
