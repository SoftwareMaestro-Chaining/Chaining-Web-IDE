...
function testBasicElements() {
  ...
  var thumb1 = new goog.ui.media.MediaModel.Thumbnail(
    "http://thumb.com/small.jpg",
    new goog.math.Size(320, 288)
  )
  var thumb2 = new goog.ui.media.MediaModel.Thumbnail(
    "http://thumb.com/big.jpg",
    new goog.math.Size(800, 600)
  )
  model.setThumbnails([thumb1, thumb2])
  model.setPlayer(
    new goog.ui.media.MediaModel.Player(
      goog.html.testing.newTrustedResourceUrlForTest("http://media/player.swf")
    )
  )
...
function testSetAriaLabel() {
  ...
  var control = new goog.ui.media.Media(model, renderer)
  assertNull(
    "Media must not have aria label by default",
  ...
  assertEquals(
    "Media element must have expected aria-label",
    "My media",
  ...
  assertEquals(
    "Media element must have updated aria-label",
    "My new media",
    element.getAttribute("aria-label")
  )
}
