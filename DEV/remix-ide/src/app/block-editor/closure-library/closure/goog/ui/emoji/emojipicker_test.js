...
function checkStructureForNonProgressivePicker(palette, emoji) {
  ...
  for (var i = 0; i < emoji[1].length; i++) {
    ...
    assertEquals(
      ...
      cell.getAttribute(goog.ui.emoji.Emoji.ATTRIBUTE),
      emojiInfo[1]
    )
    assertEquals(
      "The palette item should have the data-goomoji attribute",
      cell.getAttribute(goog.ui.emoji.Emoji.DATA_ATTRIBUTE),
      emojiInfo[1]
    )
      ...
      if (cssClass) {
        ...
        assertEquals(
          (
            ...
            .replace(/pt/g, ""),
          goog.style
            .getStyle(inner, "background-position")
            .replace(/px/g, "")
            .replace(/pt/g, "")
        )
      }
