...
function incrementVersion(db, onUpgradeNeeded) {
  ...
  return goog.db
    .openDatabase(dbName, ++dbVersion, onUpgradeNeeded, onBlocked)
...
function transactionToPromise(db, trx) {
  return new goog.Promise(function(resolve, reject) {
    ...
    goog.events.listen(trx, EventTypes.COMPLETE, function() {
      resolve(db)
    })
  })
}
    ...
    .addCallback(function(db) {
      ...
      addOverwriteTx
        ...
        .addErrback(function(err) {
          // expected
          assertEquals(goog.db.Error.ErrorName.CONSTRAINT_ERR, err.getName())
        })
...
function testPutRecordKeyPathStore() {
  ...
  return globalDb
    ...
    .addCallback(function(db) {
      var putTx = db.createTransaction(["keyStore"], TransactionMode.READ_WRITE)
      var putOperation = putTx
...
function testPutBadRecordKeyPathStore() {
  ...
  return globalDb
    ...
    .addCallback(function(db) {
      var badTx = db.createTransaction(["keyStore"], TransactionMode.READ_WRITE)
      return badTx
        .objectStore("keyStore")
...
function testPutRecordKeyPathAndAutoIncrementStore() {
  ...
  return globalDb
    ...
    .addCallback(function(db) {
      var tx = db.createTransaction(["hybridStore"], TransactionMode.READ_WRITE)
      var putOperation = tx
...
function testGetAllFreesCursor() {
  ...
  var checkStoreAndCursorDisposed = goog.partial(
    assertStoreValuesAndCursorsDisposed,
    values,
    cursors
  )
...
function testObjectStoreCursorReplace() {
  ...
  var openCursorAndReplace = function(db) {
    ...
    var whenCursorComplete = forEachRecord(cursor, function() {
      ...
      return cursor.update("5").addCallback(function() {
        cursor.next()
      })
    })

    return goog.Promise.all([cursorTx.wait(), whenCursorComplete]).then(
      function() {
        return db
      }
    )
...
function testObjectStoreCursorRemove() {
  ...
  var openCursorAndRemove = function(db) {
    ...
    var whenCursorComplete = forEachRecord(cursor, function() {
      return cursor.remove("5").addCallback(function() {
        cursor.next()
      })
    })
    return goog.Promise.all([cursorTx.wait(), whenCursorComplete]).then(
...
function testInactiveTransaction() {
  ...
  var assertCantUseInactiveTransaction = function() {
    ...
    promises.push(
      store
        .put({ key: "another", value: "thing" })
        .then(failOp("putting"), assertCorrectError)
    )
    promises.push(
      store
        .add({ key: "another", value: "thing" })
...
function testAddRecordWithIndex() {
  ...
  var addData = function(db) {
    ...
    return store
      .add({ key: "someKey", value: "lookUpThis" })
      .addCallback(function() {
        return db
      })
  }
...
function testIndexCursorReplace() {
  ...
  var openCursorAndReplace = function(db) {
    var cursorTx = db.createTransaction(["store"], TransactionMode.READ_WRITE)
    var index = cursorTx.objectStore("store").getIndex("index")
    var cursor = index.openCursor(goog.db.KeyRange.bound("3", "4", false, true))

    var cursorFinished = forEachRecord(cursor, function() {
      assertEquals("3", cursor.getValue()["value"])
      return cursor
        .update({ key: cursor.getValue()["key"], value: "5" })
        .addCallback(function() {
          cursor.next()
        })
    })
