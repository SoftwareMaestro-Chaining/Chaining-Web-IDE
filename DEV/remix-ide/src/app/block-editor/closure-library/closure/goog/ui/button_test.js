...
function testGetSetValue() {
  ...
  assertEquals(
    "Button element must have expected value",
    "17",
    button.getElement().value
  )
  button.setValue("foo")
  assertEquals(
    "Button element must have updated value",
    "foo",
    button.getElement().value
  )
  ...
  assertEquals(
    "Button element must be unchanged",
    "foo",
    button.getElement().value
  )
}
...
function testGetSetTooltip() {
  assertUndefined(
    "Button's tooltip must default to undefined",
    button.getTooltip()
  )
  button.setTooltip("Hello")
  assertEquals(
    "Button must have expected tooltip",
    "Hello",
    button.getTooltip()
  )
  button.render(sandbox)
  assertEquals(
    "Button element must have expected title",
    "Hello",
    button.getElement().title
  )
  button.setTooltip("Goodbye")
  assertEquals(
    "Button element must have updated title",
    "Goodbye",
    button.getElement().title
  )
  button.setTooltipInternal("World")
  assertEquals(
    "Button must have new internal tooltip",
    "World",
    button.getTooltip()
  )
  assertEquals(
...
function testDispose() {
  ...
  assertUndefined(
    "Button's tooltip must have been deleted",
    button.getTooltip()
  )
}
...
function testBasicButtonBehavior() {
  ...
  assertEquals(
    "Button must have dispatched ACTION on click",
    1,
    dispatchedActionCount
  )
  ...
  goog.events.unlisten(button, goog.ui.Component.EventType.ACTION, handleAction)
}
...
function testDisabledButtonBehavior() {
  ...
  assertEquals(
    "Disabled button must not dispatch ACTION on Enter key",
    0,
    dispatchedActionCount
  )
...
function testSpaceFireActionOnKeyUp() {
  ...
  button.handleKeyEvent(e)
  assertEquals(
    "Button must have dispatched ACTION on Space keyup",
    1,
    dispatchedActionCount
  )
...
function testEnterFireActionOnKeyPress() {
  ...
  button.handleKeyEvent(e)
  assertEquals(
    "Button must have dispatched ACTION on Enter keypress",
    1,
    dispatchedActionCount
  )
  ...
  button.handleKeyEvent(e)
  assertEquals(
    "Button must not have dispatched ACTION on Enter keyup",
    0,
    dispatchedActionCount
  )
...
function testSetAriaLabel() {
  assertNull(
    "Button must not have aria label by default",
    button.getAriaLabel()
  )
...
function testSetAriaLabel_decorate() {
  ...
  assertEquals(
    "Button element must have expected aria-role",
    "button",
