...
function initCard(opt_isAnchor, opt_checkChildren, opt_maxSearchSteps) {
  ...
  goog.events.listen(card, goog.ui.HoverCard.EventType.CANCEL_TRIGGER, onCancel)
  goog.events.listen(
    card,
...
function testAnchorWithChildren() {
  ...
  var inChild = new goog.math.Coordinate(
    childBounds.left + 1,
    childBounds.top + 1
  )
...
function testPositionAfterSecondTriggerWithMaxSearchSteps() {
  ...
  assertTrue("Should trigger card", card.isVisible())
  assertEquals(
    "Card cursor x coordinate should be 1",
    card.position_.coordinate.x,
    1
  )
  ...
  assertTrue("Should trigger card", card.isVisible())
  assertEquals(
    "Card cursor x coordinate should be 2",
    card.position_.coordinate.x,
    2
  )
}
