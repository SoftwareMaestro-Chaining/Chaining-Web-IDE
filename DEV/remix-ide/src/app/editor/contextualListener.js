...
class ContextualListener {
  constructor(opts, localRegistry) {
    ...
    self._deps.compiler.event.register(
      "compilationFinished",
      (success, data, source) => {
    ...
    setInterval(() => {
      this._highlightItems(
        ...
        self._deps.compiler.lastCompilationResult,
  ...
  _highlight(node, compilationResult) {
    ...
    if (eventId) {
      this._activeHighlights.push({
        eventId,
        position,
        fileTarget: self._deps.compiler.getSourceName(position.file),
        nodeId: node.id
      })
    }
  }
  ...
  _highlightInternal(position, node) {
    ...
    if (
      self._deps.compiler.lastCompilationResult &&
      self._deps.compiler.lastCompilationResult.data
    ) {
      var lineColumn = self._deps.offsetToLineColumnConverter.offsetToLineColumn(
        position,
        position.file,
        self._deps.compiler.lastCompilationResult.source.sources,
        self._deps.compiler.lastCompilationResult.data.sources
      )
