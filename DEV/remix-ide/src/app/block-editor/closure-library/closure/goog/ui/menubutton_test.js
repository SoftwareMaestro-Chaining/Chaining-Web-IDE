...
function testAlternatePositioningElement() {
  ...
  assertRoughlyEquals(
    menuNode.offsetTop,
    posElement.offsetTop + posElement.offsetHeight,
    20
  )
...
function testPositioningBelowAnchor() {
  ...
  try {
    assertRoughlyEquals(
      menuBounds.top,
      buttonBounds.top + buttonBounds.height,
      3
    )
    assertRoughlyEquals(menuBounds.left, buttonBounds.left, 3)
  } catch (e) {
...
function testSpaceFireOnKeyUp() {
  ...
  menuButton.handleKeyEvent(e)
  assertTrue(
    "Menu must have been triggered by Space keyup",
    menuButton.isOpen()
  )
