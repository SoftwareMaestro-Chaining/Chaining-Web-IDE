...
function testLabel() {
  ...
  try {
    ...
    assertTrue(
      "checkbox toggled if the label is clicked",
      checkbox.getChecked()
    )
    goog.testing.events.fireClickSequence(checkbox.getElement())
...
function testCreateDomUpdateAriaState() {
  ...
  assertEquals(
    ...
    "false",
    goog.a11y.aria.getState(checkbox.getElement(), goog.a11y.aria.State.CHECKED)
  )
...
function testSpaceKeyFiresEvents() {
  ...
  assertEquals(
    "Unexpected state after checking.",
    goog.ui.Checkbox.State.CHECKED,
    checkbox.getChecked()
  )
  ...
  events = []
  goog.events.listenOnce(checkbox, goog.ui.Component.EventType.CHECK, function(
    e
  ) {
    e.preventDefault()
  })
...
function validateCheckBox(span, state, opt_disabled) {
  ...
  assertEquals(
    "decorate was successful",
    goog.ui.Checkbox,
    testCheckbox.constructor
  )
  assertEquals(
    "checkbox state should be: " + state,
    state,
    testCheckbox.getChecked()
  )
  assertEquals(
    "checkbox is " + (!opt_disabled ? "enabled" : "disabled"),
    !opt_disabled,
