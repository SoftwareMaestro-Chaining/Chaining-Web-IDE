...
function testEscapeDismissalDoesNotRecognizeOtherKeys() {
  ...
  assertEquals(
    "Keydown, keyup, and keypress should have all propagated",
    3,
    eventsPropagated
  )
}
...
function testEscapeDismissalProvidesKeyTargetAsTargetForHideEvents() {
  ...
  goog.events.listenOnce(
    popup,
    [goog.ui.PopupBase.EventType.BEFORE_HIDE, goog.ui.PopupBase.EventType.HIDE],
    function(e) {
      calls++
      assertEquals(
        "The key target should be the hide event target",
        "targetDiv",
        e.target.id
      )
    }
