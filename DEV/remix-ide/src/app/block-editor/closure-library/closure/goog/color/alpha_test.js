...
function testHexToRgba() {
  ...
  for (var i = 0; i < badColors.length; i++) {
    var e = assertThrows(goog.partial(goog.color.alpha.hexToRgba, badColors[i]))
    assertEquals(
      "'" + badColors[i] + "' is not a valid alpha hex color",
      e.message
    )
  }
}
...
function testHexToRgbaStyle() {
  ...
  assertEquals("rgba(51,204,170,0.2)", goog.color.alpha.hexToRgbaStyle("#3CA3"))
  assertEquals(
    "rgba(1,2,3,0.016)",
    goog.color.alpha.hexToRgbaStyle("#01020304")
  )
  assertEquals(
    "rgba(255,255,0,0.333)",
...
function testRgbaArrayToHsla() {
  ...
  var nearlyOpaqueYellowHsl = goog.color.alpha.rgbaArrayToHsla(
    nearlyOpaqueYellowRgb
  )
  assertArrayEquals(
    "Conversion from RGBA to HSLA should be as expected",
    [45, 1, 0.5, 0.7],
  ...
  var transparentPurpleHsl = goog.color.alpha.rgbaArrayToHsla(
    transparentPurpleRgb
  )
  assertArrayEquals(
    "Conversion from RGBA to HSLA should be as expected",
    [282, 1, 0.5, 0],
...
function testNormalizeAlphaHex() {
  ...
  var normalizedCompactColor = goog.color.alpha.normalizeAlphaHex_(compactColor)
  assertEquals(
  ...
  var normalizedUppercaseColor = goog.color.alpha.normalizeAlphaHex_(
    uppercaseColor
  )
  assertEquals(
...
function testHsvaArrayToHex() {
  ...
  var halfTransparentPinkHex = goog.color.alpha.hsvaArrayToHex(
    halfTransparentPinkHsv
  )
  assertEquals(
    "The HSVA array should have been properly converted to hex",
    "#ff00ff7f",
  ...
  var transparentDarkTurquoiseHex = goog.color.alpha.hsvaArrayToHex(
    transparentDarkTurquoiseHsv
  )
  assertEquals(
    "The HSVA array should have been properly converted to hex",
    "#007f747f",
