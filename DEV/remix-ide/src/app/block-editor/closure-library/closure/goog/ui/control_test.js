...
function testConstructor() {
  ...
  assertEquals(
    "Content must have expected value",
    "Hello",
    control.getContent()
  )
  assertEquals(
  ...
  assertEquals(
    "DOM helper must have expected value",
    fakeDomHelper,
    foo.getDomHelper()
  )
  foo.dispose()
...
function testSetRenderer() {
  ...
  assertFalse(
    "Control still must not be in the document",
...
function testDecorateInternal() {
  ...
  assertEquals(
    "Decorated control's element must have expected value",
    foo,
    control.getElement()
  )
  assertTrue(
...
function testEnterDocument() {
  ...
  if (goog.userAgent.IE && !goog.userAgent.isVersionOrHigher(9)) {
    assertEquals(
      "Control must have 6 mouse & 3 key event listeners on IE8",
      9,
...
function testEnterDocumentForControlWithoutMouseHandling() {
  ...
  assertTrue("Control must be in the document", control.isInDocument())
  assertEquals(
    "Control must have 3 key event listeners",
    3,
    getListenerCount(control)
  )
  assertEquals(
...
function testEnterDocumentForNonFocusableControl() {
  ...
  if (goog.userAgent.IE && !goog.userAgent.isVersionOrHigher(9)) {
    assertEquals(
      "Control must have 6 mouse event listeners on IE8",
      6,
      getListenerCount(control)
    )
  } else {
    assertEquals(
      "Control must have 5 mouse event listeners",
      5,
      getListenerCount(control)
    )
  }
  assertUndefined(
    "Control must not have a key event handler",
    control.keyHandler_
  )
}
...
function testEnterDocumentForControlWithoutEventHandlers() {
  ...
  assertTrue("Control must be in the document", control.isInDocument())
  assertEquals(
    "Control must have 0 event listeners",
    0,
    getListenerCount(control)
  )
  assertUndefined(
...
function testExitDocument() {
  ...
  assertFalse(
    "Control must no longer be in the document",
    control.isInDocument()
  )
  assertEquals(
    "Control must have no event listeners",
    0,
    getListenerCount(control)
  )
  assertNull(
...
function testGetContent() {
  ...
  assertEquals(
    "Control must have expected content",
    "Hello",
    control.getContent()
  )
  control.render(sandbox)
...
function testGetContentForDecoratedControl() {
  ...
  assertEquals(
    "Content of control decorating DIV with element child " +
      "must be as expected",
    goog.dom.getElement("element").firstChild,
    element.getContent()
  )
  element.dispose()
  ...
  assertSameElements(
    ...
    goog.dom.getElement("nodelist").childNodes,
    nodelist.getContent()
  )
  nodelist.dispose()
}
...
function testSetAriaLabel_render() {
  ...
  assertEquals(
    "Element must have the new aria label",
    "new label",
    goog.a11y.aria.getLabel(elem)
  )
}
...
function testSetAriaLabel_decorate() {
  ...
  sandbox.innerHTML =
    '<div id="nodelist" role="button">' + "Hello, <b>world</b>!</div>"
...
function testSetContent() {
  ...
  assertNull(
    "Rendered control must have expected contents",
    control.getContent()
  )
  assertEquals(
    "Rendered control's DOM must have expected contents",
...
function testSetContentInternal() {
  ...
  assertEquals(
    "Control must have expected contents",
    "Bye",
    control.getContent()
  )
  assertEquals(
    "Control's DOM must be unchanged",
    "Hello",
    control.getElement().innerHTML
  )
}
...
function testGetCaption() {
  ...
  assertEquals(
    "Caption must have expected value",
    "Hello",
    control.getCaption()
  )
  ...
  var arrayContent = goog.array.clone(
    goog.dom.safeHtmlToNode(
      goog.html.testing.newSafeHtmlForTest(" <b> foo</b><i>  bar</i> ")
    ).childNodes
  )
  control.setContent(arrayContent)
  assertEquals(
    "whitespaces must be normalized in the caption",
    "foo bar",
    control.getCaption()
  )
...
function testSetAllowTextSelection() {
  ...
  control.setAllowTextSelection(true)
  assertTrue(
    "Control must allow text selection",
    control.isAllowTextSelection()
  )
...
function testSetVisibleForEnabledFocusableControl() {
  ...
  if (testFocus) {
    ...
    try {
      ...
      assertFalse(
        "Control must no longer have keyboard focus",
        control.isFocused()
      )
    } catch (e) {
      expectedFailures.handleException(e)
...
function testSetStateWithDisabled() {
  ...
  assertEquals(
    "Control element must have a tabIndex of 0",
    0,
    goog.string.toNumber(control.getElement().getAttribute("tabIndex") || "")
  )
...
function testIsHighlighted() {
  assertFalse(
    "Controls must not be highlighted by default",
...
function testSetHighlighted() {
  ...
  assertTrue(
    "Control must not have dispatched any events",
    noEventsDispatched()
  )
...
function testSetSelected() {
  ...
  control.setSelected(true)
  assertTrue(
    "Control must be selected, even when disabled",
    control.isSelected()
  )
  assertEquals(
...
function testGetState() {
  assertEquals(
    "Controls must be in the default state",
    0x00,
    control.getState()
  )
}
...
function testSetPointerEventsEnabled() {
  ...
  var pointerup = new goog.testing.events.Event(
    goog.events.PointerFallbackEventType.POINTERUP,
    control.getElement()
  )
